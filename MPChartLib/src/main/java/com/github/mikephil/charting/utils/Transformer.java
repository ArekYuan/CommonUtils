...
public class Transformer {
    ...
    /**
     * 初始化值 避免nullPointException
     */
    protected float[] valueHighPoints = new float[1];

    /**
     * 获取high值的坐标位置
     *
     * @return 数组 float[] 像素坐标，x,y
     */
    public float[] getValueHighPoints() {
        getValueToPixelMatrix().mapPoints(this.valueHighPoints);//将数值转化成 坐标点
        return this.valueHighPoints;
    }

    /**
     * 绑定high值的坐标
     *
     * @param valueHighPoints
     */
    public void setValueHighPoints(float[] valueHighPoints) {
        this.valueHighPoints = valueHighPoints;
    }
     ...
     *
     * @param data
     * @return 将条目item转化为x 轴 y轴坐标,取low值
     */
    public float[] generateTransformedValuesCandle(ICandleDataSet data,
        ...
        if (this.valueHighPoints.length != count) {
            this.valueHighPoints = new float[count];
        }
        float[] valuePoints = valuePointsForGenerateTransformedValuesCandle;
        float[] valueHighPoints = this.valueHighPoints;
        for (int j = 0; j < count; j += 2) {
            ...
            if (e != null) {
                valuePoints[j] = e.getX();
                valuePoints[j + 1] = e.getLow() * phaseY;//将low数值转化为坐标点
                valueHighPoints[j] = e.getX();
                valueHighPoints[j + 1] = e.getHigh() * phaseY;
                setValueHighPoints(valueHighPoints);
            } else {
                valuePoints[j] = 0;
                valuePoints[j + 1] = 0;
                valueHighPoints[j] = 0;
                valueHighPoints[j + 1] = 0;
                setValueHighPoints(valueHighPoints);
            }
        }
