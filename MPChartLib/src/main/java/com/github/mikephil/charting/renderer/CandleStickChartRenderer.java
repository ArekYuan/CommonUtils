    ...
    public void drawValues(Canvas c) {
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                Log.i("ICandleDataSet--->", "-yMin-->" + yMin + ",,yMax--->" + yMax);
                ...
                float[] positionYHigh = trans.getValueHighPoints();
                Log.i("ICandleDataSet--->", "YLow-->" + positions.length + ",,YHigh---->" + positionYHigh.length + "<---");
                float yOffset = Utils.convertDpToPixel(10f);
                float xOffset = Utils.convertPixelsToDp(8f);
                MPPointF iconsOffset = MPPointF.getInstance(dataSet.getIconsOffset());
                iconsOffset.x = Utils.convertDpToPixel(iconsOffset.x);
                iconsOffset.y = Utils.convertDpToPixel(iconsOffset.y);
                Log.i("iconsOffset.x-->", iconsOffset.x + "");
                Log.i("iconsOffset.y-->", iconsOffset.y + "");
                for (int j = 0; j < positions.length; j += 2) {

                    float x = positions[j];
                    float y = positions[j + 1];
                    float h = positionYHigh[j + 1];
                    ...
                    if (dataSet.isDrawValuesEnabled()) {
                        if (dataSet.getYMin() == entry.getLow()) {//自动计算Y轴最低值，
                        ...
                        if (dataSet.getYMax() == entry.getHigh()) {
                            drawValue(c,
                                    dataSet.getValueFormatter(),
                                    entry.getHigh(),
                                    entry,
