...
public class ConnectSersvice
{
     ...
     */
    public <T> void connectService(HttpRequestParams httpRequestParams, final UICallBack callback, Class<T> obj,
...
//        String serviceUrl = CommonConstants.SERVER + methodName;
    ...
    private <T> void doJsonRequest(HttpRequestParams httpRequestParams, final INetCallBack netBack, final Class<T> obj,
                                   final Context context, final String serviceUrl)
    {
        // 请求参数
        final Map<String, String> reqParams = new HashMap<String, String>();
        // 请求方法
      HttpRequestParams.HttpRequestMethod httpRequestMethod = httpRequestParams.getHttpRequestMethod();
        ...
        Response.ErrorListener errorListener = new Response.ErrorListener()
        {
            ...
            public void onErrorResponse(VolleyError error)
            {
                ...
                Log.e("connect server error", "error message:" + error);
            }
        };
        Response.Listener<JSONObject> listener = new Response.Listener<JSONObject>()
        {
            ...
            public void onResponse(JSONObject response)
            {
                try
                {
                    ...
                    if (code == 2000)
                    {
                        Log.e("请求成功---->url:" + url + ":-->", response.toString());
                        Gson gson = new Gson();
                    ...
                    {
                        Log.e("服务器处理失败---->url:" + url + ":-->", response.toString());
        ...
        if (TextUtils.isEmpty(httpRequestParams.getTag()))
        {
         ...
         RequestQueueHandler.getInstance().addRequest(handler);
    }
    ...
    public <T> void connectGetService(final String tag, final  UICallBack callback, final Class<T> obj,
...
//        String serviceUrl = CommonConstants.SERVER + methodName;
       ...
       final String url = bindParamsIfNeed(Request.Method.GET, serviceUrl, reqParams);
        ...
        Response.ErrorListener errorListener = new Response.ErrorListener()
        {
            ...
            public void onErrorResponse(VolleyError error)
            {
                onFail("链接服务器失败", netBack);
                error.printStackTrace();
                Log.e(tag + "---->doJsonRequestGet", "链接服务器失败");
            }
        };
        Response.Listener<JSONObject> listener = new Response.Listener<JSONObject>()
        {
            ...
            public void onResponse(JSONObject response)
            {
                ...
                try
                {
                    ...
                    if (code == 2000)
                    {
                        Log.e(tag + "请求成功---->url:" + url + ":-->", response.toString());
                        Gson gson = new Gson();
                        Object object = gson.fromJson(response.toString(), obj);
                        onSuccess(object, netBack);
                    }
                    ...
                    {
                        Log.e(tag + "服务器处理失败---->url:" + url + ":-->", response.toString());
                        String message = response.getString("message");
                        onReqFailed(message, netBack);
                    }
     ...
     * @return 返 回 类 型：int
     */
    public int getMethod(HttpRequestParams.HttpRequestMethod httpRequestMethod)
    {
        int method = -1;
