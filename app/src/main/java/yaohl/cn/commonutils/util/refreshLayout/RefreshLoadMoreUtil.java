...
public class RefreshLoadMoreUtil
{
    ...
    private static final String TAG = "RefreshLoadMoreUtil";
    ...
    public static boolean isContentToTop(View view)
    {
        ...
        else if (view instanceof AbsListView)
        {
            ...
            if (0 == listView.getFirstVisiblePosition())
            {//此时没有完全显示
    ...
    public static boolean isContentToItem(RecyclerView recyclerView, boolean isFirst, int targetIndex, Type type)
    {
        ...
        {
            CMLog.e(TAG, "Invalid targetIndex = %d");
            return false;
        }
        else
        {
            CMLog.i(TAG, "targetIndex = %d");
        }
        int[] poss = getRecyclerViewFirstCompleteVisiblePos(recyclerView, true, isFirst);
        boolean match = false;
        for (int p : poss)
        {
            CMLog.v(TAG, "complete visible position = %d");
            if (targetIndex == p)
            {
                match = true;
                break;
            }
            else if (RecyclerView.NO_POSITION == p)
            {//NO_POSITION,比如第一个view高度很高,这种情况就没有完全显示的item
                if (type == Type.TOP && 0 == targetIndex)
                {
                    ...
                    else
                    {
                        ...
                        CMLog.v(TAG, "first item view top = %d, topMargin = %d");
                ...
                else if (type == Type.BOTTOM && adapItemCount - 1 == targetIndex)
                {
                    ...
                    else
                    {
                        ...
                        CMLog.v(TAG, "bottom item view bottom = %d, bottomMargin = %d, recyclerview height = %d");
