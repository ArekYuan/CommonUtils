...
public class RefreshLoadMoreUtil {
    ...
    public static boolean isContentToItem(RecyclerView recyclerView, boolean isFirst, int targetIndex, Type type) {
        ...
        if (targetIndex < 0 || targetIndex >= adapItemCount) {
            YLog.d("Invalid targetIndex = %d");
            return false;
        } else {
            YLog.i("targetIndex = %d");
        }
        int[] poss = getRecyclerViewFirstCompleteVisiblePos(recyclerView, true, isFirst);
        boolean match = false;
        for (int p : poss) {
            YLog.v("complete visible position = %d");
            if (targetIndex == p) {
                match = true;
                break;
                ...
                if (type == Type.TOP && 0 == targetIndex) {
                    ...
                    if (null == viewHolder) {
                        ...
                        YLog.v("bottom item view bottom = %d, bottomMargin = %d, recyclerview height = %d");
